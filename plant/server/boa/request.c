...
int process_header_end(request * req) {
	...
	if (req->is_html) {
		...
		//not index page
		int iRole = 0;
		if (!checkLoginStaues(req->remote_ip_addr, &iRole)) {
			//not login
			fprintf(stderr, "===========not login===========\n");

			char tmp_pathName[32];
			sprintf(tmp_pathName, "%s/login.html", document_root);
			//fprintf(stderr,"Before modified  pathname ==%s\n",req->pathname);
			free(req->pathname);
			req->pathname = strdup(tmp_pathName);
			//fprintf(stderr,"modified  pathname ==%s\n",req->pathname);
		} else {
			if (iRole < 2) {
				//replace header.html menu file
				char *p = strstr(req->pathname, "header.html");
				if (p != NULL ) {
					fprintf(stderr, "===========role changed===========\n");

					char tmp_pathName[32];
					sprintf(tmp_pathName, "%s/header%d.html", document_root,iRole);
					//fprintf(stderr,"Before modified  pathname ==%s\n",req->pathname);
					free(req->pathname);
					req->pathname = strdup(tmp_pathName);
					fprintf(stderr,"modified  pathname ==%s\n",req->pathname);
