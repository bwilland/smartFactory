...
int translate_uri(request * req) {
	...
	/********************* add by bill for actions********************/
		char index_html[32];
		sprintf(index_html, "%s/", document_root);

		if (strcmp(buffer, index_html) == 0) {
			//index.html
			strcat(buffer, directory_index);
		...
		if (strstr(buffer, ".action")) {
			...
			if (strstr(buffer, ".html")) {
				req->is_html = 1;
				//fprintf(stderr, "*************HTML alias********** %s\n", req->pathname);
			}
		}

		/********************* add by bill for actions END********************/
...
#endif
	if (strncmp("nph-", pathname + current1->real_len, 4) == 0 || req->simple)
		req->is_cgi = NPH;
	else
		req->is_cgi = CGI;
		...
		while (current && !req->path_translated) {
			if (!strncmp(req->path_info, current->fakename,
				...
				if (current->real_len + path_len
						- current->fake_len> MAX_HEADER_LENGTH) {
					log_error_doc(req);
					fputs("uri too long!\n", stderr);
					send_r_bad_request(req);
					return 0;
